...
local ESPDistanceOps = {50.0, 100.0, 500.0, 1000.0, 2000.0, 50000.0, 100000.0}
...
Citizen.CreateThread(
	function()
		...
		proxy.CreateMenu("MainMenu", "∑ Galaxy ~n~~y~MENU ∑")
		proxy.SetSubTitle("MainMenu", "VERSION 3.1")
        ...
        while bm do
			...
			if proxy.IsMenuOpened("MainMenu") then
			    ...
			    drawNotification(" ∑ Galaxy Reborn 3.0 ∑  ~n~~h~WELCOME ~r~[~s~ " ..GetPlayerName(PlayerId()).." ~r~]")
			    drawNotification("~h~Have fun while using this menu | Version Is: ~h~~g~3.0 ~s~Made By ~r~Balaclava™")
				drawNotification("~h~~g~Official Discord | DevilMachine | ~s~~n~~h~for problem dm kyperbot")
				...
				elseif proxy.Button("X ~r~UnloadMenu ~s~X") then
					bm = false
				end
		...
		proxy.Display()
			...
			elseif proxy.Button("~h~Teleport ~s~~p~To Coords") then
                TeleportToCoords()
			elseif proxy.Button("~h~Teleport ~h~~g~Police ") then
			    LSPD()
		    ...
		    elseif proxy.Button('~h~Teleport ~h~~g~Ammunation') then
			    Ammunation()
			elseif proxy.Button('~h~Teleport ~h~~g~Batman Spilia') then
			    shopclothes()
			elseif proxy.Button('~h~Teleport ~h~~g~Platia') then
			    barber()
			elseif proxy.Button('~h~Teleport ~h~~g~Maze Bank') then
			    MazeBank()
			elseif proxy.Button('~h~Teleport ~h~~g~Spot') then
			    spot()
			elseif proxy.Button('~h~Teleport ~h~~g~1v1 Logo') then
			    MichaelHouse()
			elseif proxy.Button('~h~Teleport ~h~~g~paleto 1055') then
			    trevor()
    ...
    TriggerEvent('esx_skin:openSaveableMenu')
			...
			elseif proxy.IsMenuOpened("PlayerOptionsMenu") then
                ...
                elseif proxy.Button("~h~~r~Spawn ~s~Swat army with ~y~AK") then
					...
					elseif proxy.Button("~h~~r~Spawn big robot ~s~ with ~y~hgr") then
                    local bQ = "u_m_y_juggernaut_01"
                    local bR = "weapon_hominglauncher"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                        RequestModel(GetHashKey(bQ))
                        Citizen.Wait(50)
